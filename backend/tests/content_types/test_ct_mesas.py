...
class TestFTI:
    ...
    @pytest.mark.parametrize(
        "attribute,expected",
        (
            ("title", "Mesas Diretoras"),
            ("global_allow", True),
            ("filter_content_types", True),
            ("allowed_content_types", ()),
            ("klass", "sc.eprocessos.content.mesas.Mesas"),
            ("add_permission", "sc.eprocessos.AddMesas"),
...
class TestContentType:
    ...
    def test_instance_class(self):
        ...
        assert isinstance(self.content, Mesas)
        assert isinstance(self.content, EProcessosFacade)

    def test_provides_marker_interface(self):
        """Content provides both IMesas and IEProcessosFacade."""
        assert IMesas.providedBy(self.content)
